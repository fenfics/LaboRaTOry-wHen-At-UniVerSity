...
public class Family extends JPanel {
    ...
    private JRadioButton divorce_mother1, divorce_father1, divorce_havekid, divorce_donthavekid, divorce_father2,
            divorce_mother2, divorce_cousin1, divorce_son, divorce_father3, divorce_mother3, divorce_cousin2;
    ...
    public Family(MainApplication mainApp) {
        ...
        layeredPane0.setSize(1133, 744);
        // layeredPane0.getContentPane().setBackground(Color.decode("#D8F5FF"));
        layeredPane0.setLayout(null);
        ...
        button.addActionListener(e -> mainApp.showPage("Home"));
        ...
        button1.addActionListener(e -> mainApp.showPage("Salary"));
        ...
        button2.addActionListener(e -> {
            int totalDeduction = getTotalDeduction(); // ดึงค่า totalDeduction
            mainApp.sendValuesToPage12(Salaryyear, totalDeduction); // ส่งค่าไปหน้า Page12
            mainApp.showPage("page12"); // แสดงหน้า Page12
        });
    ...
    private JPanel createMarriedHaveMoneyPanel() {
        ...
        // ปุ่มกลมพ่อแม่ญาติ
        MHM_father4 = new JRadioButton(" บิดา");
    ...
    // คำนวณลดหย่อนผู้พิการหรือทุพพลภาพ (ไม่มีเงินได้) &&
    // ลดหย่อนผู้พิการหรือทุพพลภาพ (คู่สมรสไม่มีเงินได้)
    ...
    public int getTotalDeduction() {
        String totalDeductionText = totalDeductionField.getText(); // ดึงค่าจาก JTextField
        if (totalDeductionText.isEmpty()) {
            return 0; // ถ้าไม่มีค่าในช่อง ให้รีเทิร์น 0
        }
        // ลบ "," และแปลงเป็น int
        return Integer.parseInt(totalDeductionText.replace(",", ""));
    }

    public void receiveSalary(int salaryAfterExpenses) {
        this.Salaryyear = salaryAfterExpenses;

        // คำนวณค่าหลังจากหักการหักลบ
        int totalDeduction = getTotalDeduction();

        // ส่งค่า salaryAfterExpenses และ totalDeduction ไปยังหน้า Page12
        mainApp.sendValuesToPage12(salaryAfterExpenses, totalDeduction);
    }
